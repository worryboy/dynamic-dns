...
- Verification design notes: [../spec/design/verification.md](../spec/design/verification.md)
...
Set `LOG_TARGET` only if you want logs written somewhere else, such as `/app/state/dynamic-dns-worker.log`.
...
Use `docker compose run --rm dynamic-dns-worker` when `RUN_ONCE=true`. Do not use `docker compose up -d` for one-shot validation with the default restart policy, because Docker will restart the cleanly exited container and it can look like a failure loop.
...
- `RUN_ONCE=true` executes a single check cycle and exits instead of looping continuously. It is meant for `docker compose run --rm dynamic-dns-worker`.
...
The current primary check is provider-side ZoneInfo before update decisions. A future verification pass should stay staged and optional: provider check first, configurable public resolver checks second, and local resolver checks only as a convenience. See [../spec/design/verification.md](../spec/design/verification.md).
...
The compose setup contains one service: `dynamic-dns-worker`.
...
- [`../spec/design/verification.md`](../spec/design/verification.md)
